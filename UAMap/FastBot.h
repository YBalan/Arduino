...
// постучимся ещё раз
        ...
        uint8_t status = 1;
		BOT_TRACE(F("HTTP ANSWER: "), answ);
		BOT_TRACE(F("HTTP ANS SIZE: "), _http->getSize());		
        if (answ == HTTP_CODE_OK && _http->getSize()) {  // есть ответ и он не пустой			
			auto res = _http->getString();
            parseRequest(res);            // парсим
			BOT_TRACE(F("HTTP ANS STR: "), res);
        } else {
			BOT_TRACE(F("HTTP ANS STR: "), _http->getString());
			status = 3;                               // некорректный ответ
		}
        _http->end();
        return status;
    }
